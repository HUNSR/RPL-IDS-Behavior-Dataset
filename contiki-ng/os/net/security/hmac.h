...
#include <stdlib.h>
#include <string.h>
// #define SHA256_HASH_SIZE 12
#define SHA256_HASH_SIZE 4
...
// unsigned int get32(uint8_t *buffer, int pos)
// {
//     return ((uint32_t)buffer[pos] << 24 | (uint32_t)buffer[pos + 1] << 16 |
//             (uint32_t)buffer[pos + 2] << 8 | buffer[pos + 3]);
// }

unsigned int getInt(uint8_t *buffer, int pos)
{
    return ((uint32_t)buffer[pos] << 24 | (uint32_t)buffer[pos + 1] << 16 |
            (uint32_t)buffer[pos + 2] << 8 | buffer[pos + 3]);
}
...
unsigned int hmac_sha256(const void *key,
                         const size_t keylen,
                         const void *data,
                         const size_t datalen)
{
